...
export class ChapterSessionFirebaseRepository implements IChapterSessionPort {
  ...
  async getSession(uid: string, desafioId: string, capituloId: number): Promise<ChapterSession | null> {
    try {
      ...
      console.error(`[Firebase] ChapterSession getSession: requires Admin SDK credentials — ${(err as Error).message}`);
      return null;
    }
  }
  ...
  async saveSession(
    ...
    try {
      ...
      console.error(`[Firebase] ChapterSession saveSession: requires Admin SDK credentials — ${(err as Error).message}`);
      return emptySession(uid, desafioId, capituloId);
    }
  }
}
