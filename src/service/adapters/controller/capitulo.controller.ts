...
import { CapituloView, ObjetivoComConsulta } from "../../core/domain/capitulo.entity";
...
export const getCapituloViewById = asyncHandler(async (req: Request, res: Response<ApiResponse<CapituloView>>) => {
    const capituloId = Number(req.params.id);
    if (isNaN(capituloId)) throw new ValidationError("ID inválido. Deve ser um número.");
